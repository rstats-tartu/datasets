...
pull_maaamet <- . %>%
  ...
  mutate(
    url = map2_chr(algus, lopp, ~sprintf(baseurl, aru, ehak, .x, .y)),
    resp = map(url, ~{
      message(str_trunc(.x, 80, "center"))
      try(read_html(.x))
    })
  )
...
periods <- data_frame(
  start = seq(dmy("01-01-2005"), dmy("01-01-2018"), by = "2 years"),
...
res_ap <- residential_apartments %>%
  mutate(
    Month = month(dmy(lopp), label = TRUE),
    Year = year(dmy(lopp))
  ) %>%
